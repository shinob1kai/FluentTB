...
namespace FluentTB
{
    public class Background
    {
        ...
        private bool _nativeAutoHideActive;

        public Background(MainWindow owner)
        {
            mw = owner;
        }
        ...
        public void DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            while (!worker.CancellationPending)
            {
                ...
                catch (TypeInitializationException ex)
                {
                    ...
                    TaskbarDiagnostics.LogCrash("Background.DoWork TypeInitializationException", ex);
                    throw;
                }
                catch (Exception ex)
                {
                    mw.interaction.AddLog($"Background tick error: {ex.Message}");
                    TaskbarDiagnostics.Log($"Background tick error: {ex.GetType().Name}: {ex.Message}");
                }
        ...
        private void RunPerTickTasks()
        {
            ...
            {
                if (mw.activeSettings == null || !mw.activeSettings.TaskbarShapeEnabled)
                    return;
            ...
            // Windows' native auto-hide and SetWindowRgn are incompatible.  Reset
            // once when it is enabled, then suspend changes until Explorer's
            // auto-hide setting is turned off again. This prevents the persistent
            // flickering documented by RoundedTB.
            if (Interaction.IsAutoHideEnabled())
            {
                if (!_nativeAutoHideActive)
                {
                    _nativeAutoHideActive = true;
                    foreach (Types.Taskbar taskbar in taskbars)
                        Taskbar.ResetTaskbar(taskbar, settings);
                    mw.interaction.AddLog("Windows native auto-hide is enabled; taskbar styling is suspended.");
                }
                return;
            }
            ...
            if (_nativeAutoHideActive)
            {
                _nativeAutoHideActive = false;
                shellDirty = true;
                mw.interaction.AddLog("Windows native auto-hide was disabled; taskbar styling resumed.");
            }
            ...
            for (int i = 0; i < taskbars.Count; i++)
            {
                ...
                if (shouldFill)
                {
                    // The media widget still needs current placement when the
                    // taskbar is filled for a maximised window.
                    if (geometryChanged)
                    {
                        tb.TaskbarRect = fresh.TaskbarRect;
                        tb.AppListRect = fresh.AppListRect;
                        tb.TrayRect = fresh.TrayRect;
                        tb.WidgetsRect = fresh.WidgetsRect;
                        tb.ScaleFactor = fresh.ScaleFactor;
                        anyChange = true;
                    }
                    if (!wasIgnored)
                    {
                ...
                bool useDynamic = settings.IsDynamic && Taskbar.CanUseDynamicTaskbar(tb);
                ...
                {
                    if (!Taskbar.UpdateDynamicTaskbar(tb, settings, forceAccentReset: wasIgnored))
                        Taskbar.UpdateSimpleTaskbar(tb, settings, forceAccentReset: wasIgnored);
                    if (_infrequentCount == 0)
        ...
        private static Types.Taskbar CloneTaskbar(Types.Taskbar taskbar)
        {
            ...
            return new Types.Taskbar
            {
                ...
                WidgetsRect = taskbar.WidgetsRect,
                WidgetsHwnd = taskbar.WidgetsHwnd,
                ClockRect = taskbar.ClockRect,
                ClockHwnd = taskbar.ClockHwnd,
