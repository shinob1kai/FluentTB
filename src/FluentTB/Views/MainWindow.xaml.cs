...
namespace FluentTB
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private bool _nativeAutoHideWarningShown;
        ...
        public MainWindow(bool hosted = false)
        {
            ...
            if (!IsHosted && matchingProcesses.Length > 1)
            {
...
#pragma warning restore CS4014
            ...
            // and fill in any settings that didn't exist in older configs
            // (they default to false/0 after JSON deserialisation, which is
            // wrong for fields that should default to true).
            if (activeSettings.Version != AppVersion && AppVersion != -1)
            {
                activeSettings.IsNotFirstLaunch = false;

                // Back-fill booleans that should default to true but are
                // serialised as false in configs written before they existed.
                var defaults = BuildDefaultSettings();
                if (!activeSettings.FillOnMaximise && !activeSettings.FillOnTaskSwitch)
                {
                    // Both are false — likely an old config; apply defaults.
                    activeSettings.FillOnMaximise   = defaults.FillOnMaximise;
                    activeSettings.FillOnTaskSwitch = defaults.FillOnTaskSwitch;
                }
            }
            ...
            {
                marginSlider.Value = activeSettings.MarginBasic;
            ...
            cornerRadiusSlider.Value         = activeSettings.CornerRadius;
            cornerRadiusInput.Text           = activeSettings.CornerRadius.ToString();
            musicEnabledCheckBox.IsChecked = activeSettings.MusicWidgetEnabled;
            musicArtistCheckBox.IsChecked = activeSettings.MusicWidgetShowArtist;
            musicControlsCheckBox.IsChecked = activeSettings.MusicWidgetControls;
            musicIdleCheckBox.IsChecked = activeSettings.MusicWidgetHideWhenIdle;
            musicMonitorsCheckBox.IsChecked = activeSettings.MusicWidgetAllMonitors;
            widgetsCheckBox.IsChecked = activeSettings.ShowWidgets;
            ...
            if (!IsHosted && activeSettings.IsNotFirstLaunch != true)
            {
        ...
        {
            if (IsHosted) { TrayIcon.Visibility = Visibility.Collapsed; return; }
            string name = ReadSystemUsesLightTheme() ? "TrayLight.ico" : "TrayDark.ico";
            var uri = new Uri("pack://application:,,,/FluentTB.Taskbar;component/res/" + name);
        ...
        public void ApplyButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            bool wasShapeEnabled = activeSettings.TaskbarShapeEnabled;
            ...
            pendingSettings.MusicWidgetEnabled = musicEnabledCheckBox.IsChecked == true;
            pendingSettings.MusicWidgetShowArtist = musicArtistCheckBox.IsChecked == true;
            pendingSettings.MusicWidgetControls = musicControlsCheckBox.IsChecked == true;
            pendingSettings.MusicWidgetHideWhenIdle = musicIdleCheckBox.IsChecked == true;
            pendingSettings.MusicWidgetAllMonitors = musicMonitorsCheckBox.IsChecked == true;
            pendingSettings.ShowWidgets = widgetsCheckBox.IsChecked == true;
            lock (TaskbarStateLock)
            {
            ...
            if (!appliedSettings.TaskbarShapeEnabled)
            {
                _workerRestartPending = false;
                _shapeResetPending |= wasShapeEnabled || taskbarThread.IsBusy;
                _autoHideMgr.Stop();
                if (taskbarThread.IsBusy) taskbarThread.CancelAsync();
                else CompleteShapeReset();
                interaction.WriteJSON();
                return;
            }

            if (Interaction.IsAutoHideEnabled())
            {
                interaction.WriteJSON();
                ResetTaskbarsForNativeAutoHide();
                return;
            }
            ...
            try
            {
                ...
                {
                    // Re-sample Explorer immediately before applying. This avoids
                    // constructing a dynamic region from the empty/stale AppList
                    // rectangle that is reported during an alignment transition.
                    Types.Taskbar fresh = Taskbar.GetQuickTaskbarRects(
                        taskbar.TaskbarHwnd, taskbar.TrayHwnd, taskbar.AppListHwnd);
                    taskbar.TaskbarRect = fresh.TaskbarRect;
                    taskbar.AppListRect = fresh.AppListRect;
                    taskbar.TrayRect = fresh.TrayRect;
                    taskbar.WidgetsRect = fresh.WidgetsRect;
                    taskbar.ScaleFactor = fresh.ScaleFactor;

                    bool useDynamic = appliedSettings.IsDynamic &&
                                      Taskbar.CanUseDynamicTaskbar(taskbar);
                    if (!useDynamic)
                        Taskbar.UpdateSimpleTaskbar(taskbar, appliedSettings, forceAccentReset: true);
                    else if (!Taskbar.UpdateDynamicTaskbar(taskbar, appliedSettings, forceAccentReset: true))
                        Taskbar.UpdateSimpleTaskbar(taskbar, appliedSettings, forceAccentReset: true);
                }
        ...
        private void ResetTaskbarsForNativeAutoHide()
        {
            List<Types.Taskbar> snapshot;
            ...
            interaction.AddLog("Apply skipped because Windows native auto-hide is enabled.");
            if (_nativeAutoHideWarningShown)
                return;

            _nativeAutoHideWarningShown = true;
            MessageBox.Show(
                "Windows taskbar auto-hide is enabled. FluentTB pauses its taskbar " +
                "changes because Windows auto-hide and rounded taskbar regions cause " +
                "persistent flickering together. Disable auto-hide in Windows Settings, " +
        ...
        // â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        private bool _shapeResetPending;
        private void CompleteShapeReset()
        {
            if (!_shapeResetPending) return;
            _shapeResetPending = false;
            ResetAllTaskbarRegions();
            CloseTaskbarShadows();
        }
        ...
        {
            CompleteShapeReset();
            if (_workerRestartPending && activeSettings.TaskbarShapeEnabled && !shouldReallyDieNoReally)
            {
                _workerRestartPending = false;
                var a = _pendingArgs;
        ...
        protected override void OnClosing(CancelEventArgs e)
        {
            ...
            _autoHideMgr.Stop();
            if (activeSettings.TaskbarShapeEnabled || _shapeResetPending) ResetAllTaskbarRegions();
            CloseTaskbarShadows();
        ...
        private void ResetAllTaskbarRegions()
        {
            try
            {
                List<Types.Taskbar> snapshot;
            ...
            {
                interaction?.AddLog($"Taskbar reset on exit failed: {ex.Message}");
            }
        }
        ...
        */
        ...
        private void advancedMarginsButton_Click(object sender, RoutedEventArgs e)
        {
            ...
            advancedMarginsButton.Content = enabling ? "Use one margin" : "Use individual margins";
        ...
        private static Types.Settings CopySettings(Types.Settings src)
        {
            return new Types.Settings
            {
                TaskbarShapeEnabled = src.TaskbarShapeEnabled,
                Version           = src.Version,
                ...
                ShowWidgets       = src.ShowWidgets,
                ShowClock         = src.ShowClock,
                ShowSegmentsOnHover = src.ShowSegmentsOnHover,
                MusicWidgetEnabled = src.MusicWidgetEnabled,
                MusicWidgetShowArtist = src.MusicWidgetShowArtist,
                MusicWidgetControls = src.MusicWidgetControls,
                MusicWidgetHideWhenIdle = src.MusicWidgetHideWhenIdle,
                MusicWidgetAllMonitors = src.MusicWidgetAllMonitors,
            };
        }
        ...
        public void SyncTaskbarShadows(List<Types.Taskbar> taskbars, Types.Settings settings)
        {
            ...
            {
                if (!activeSettings.TaskbarShapeEnabled || !settings.ShowTaskbarShadow || taskbar.Ignored || taskbar.EffectiveRegion == null)
                {
