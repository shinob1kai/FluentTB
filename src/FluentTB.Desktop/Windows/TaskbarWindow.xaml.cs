...
public partial class TaskbarWindow : Window
{
    ...
    private int _widgetCornerDiameter;
    ...
    private bool? _lastCenteredAlignment;
    ...
    public TaskbarWindow()
    {
        ...
        IsVisibleChanged += (s, e) =>
        {
            ...
            if (handle != IntPtr.Zero) FluentTB.ExternalTaskbarContent.Register(handle);
        };
        Closed += (s, e) => FluentTB.ExternalTaskbarContent.Unregister(new WindowInteropHelper(this).Handle);
    ...
    private void UpdateWindowRegion(IntPtr windowHandle, params Rect[] rects)
    {
        ...
        foreach (var r in rects)
        {
            ...
            IntPtr newRgn = CreateRoundRectRgn((int)r.Left, (int)r.Top, (int)r.Right, (int)r.Bottom,
                _widgetCornerDiameter, _widgetCornerDiameter);
        ...
        // Publish only after the child region is ready for the parent to copy.
        FluentTB.ExternalTaskbarContent.Register(windowHandle);
...
// All regions that were not sent without errors to SetWindowRgn must be destroyed manually
    ...
    private bool ReadCenteredAlignment()
    {
        try
        {
            using var key = Microsoft.Win32.Registry.CurrentUser.OpenSubKey(
                @"Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced");
            // Windows 11 defaults to centered when the preference has never been written.
            return key?.GetValue("TaskbarAl") is int alignment ? alignment != 0 : true;
        }
        catch (Exception ex) when (ex is System.Security.SecurityException or UnauthorizedAccessException or System.IO.IOException)
        {
            return _lastCenteredAlignment ?? true;
        }
    }
    ...
    private Rect PositionWidget(IntPtr taskbarHandle, RECT taskbarRect, double dpiScale, bool isMainTaskbarSelected, bool isVertical)
    {
        ...
        var shape = new FluentTB.TaskbarWidgetShape(FluentTB.Integration.TaskbarIntegration.Engine.activeSettings.Clone(), dpiScale, crossSize, isVertical);
        physicalHeight = shape.Height;
        int crossPos = shape.Top;
        _widgetCornerDiameter = shape.CornerDiameter;
        // GDI accepts an ellipse diameter; WPF CornerRadius accepts its radius.
        Widget.ApplyTaskbarShape(shape.CornerDiameter / (2 * dpiScale));
        if (physicalHeight == 0) return Rect.Empty;

        // Primary axis position (calculated per-case below)
        int primaryPos = 0;

        // Refresh on every position update, including startup and live alignment changes.
        bool centered = ReadCenteredAlignment();
        if (_lastCenteredAlignment != centered)
        {
            _lastCenteredAlignment = centered;
            _widgetElement = null;
            _trayElement = null;
            _taskbarFrameElement = null;
        }
        int position = TaskbarWidgetPlacement.Resolve(
            SettingsManager.Current.TaskbarWidgetFollowAlignment, centered,
            SettingsManager.Current.TaskbarWidgetPosition, isVertical);
        switch (position)
        {
        ...
        primaryPos = Math.Clamp(primaryPos + SettingsManager.Current.TaskbarWidgetManualPadding,
            0, Math.Max(0, primarySize - physicalWidth));
