...
namespace FluentFlyoutWPF.Classes
{
    public class Visualizer : IDisposable
    {
        ...
        {
            if (_captureRequested == enabled) return;
            _captureRequested = enabled;
            Logger.Info("Media widget visualizer requested: {0}", enabled);
            _recovery.SetEnabled(enabled);
            if (enabled) _captureWatchdog?.Start();
            else
            {
        ...
        public Visualizer()
        {
            _recovery = new CaptureRecovery(Start, Stop);
            _captureWatchdog = new System.Timers.Timer(500) { AutoReset = true };
            _captureWatchdog.Elapsed += (_, _) => ClearSilentBars();
            InitializeBitmap();
        ...
        public void RefreshDevice() => RequestRestart("selected visualization output changed");
        ...
        private void RequestRestart(string reason)
        {
            ...
            Logger.Info("Visualizer recovery requested: {0}", reason);
            _recovery.RequestRestart();
        }

        private void ClearSilentBars()
        {
            if (!_captureRequested || !_isRunning ||
                DateTime.UtcNow - _lastDataAvailableUtc < TimeSpan.FromMilliseconds(500)) return;
            var bars = _barValues;
            if (bars != null) Array.Clear(bars);
            UpdateBitmap();
            if (!SettingsManager.Current.TaskbarVisualizerBaseline || SettingsManager.Current.TaskbarVisualizerBaselineAutoHide)
                SettingsManager.Current.TaskbarVisualizerHasContent = false;
            // WASAPI can legitimately send no packets during silence. Recovery is driven
            // by failed starts, RecordingStopped and device/session notifications instead.
        }
        ...
        private bool Start()
        {
            ...
            try
            {
                ...
                var deviceId = SettingsManager.Current.TaskbarVisualizerDeviceId;
                _renderDevice = string.IsNullOrWhiteSpace(deviceId)
                    ? AudioDeviceMonitor.Instance.GetDefaultRenderDevice()
                    : AudioDeviceMonitor.Instance.GetDeviceById(deviceId);
            ...
            {
                Logger.Error(ex, "Failed to start visualizer; retrying while enabled");
                return false;
            }
        }
        ...
        private void Stop()
        {
            ...
            if (capture != null)
            {
                ...
                catch (Exception ex) { Logger.Warn(ex, "Failed to stop visualizer capture"); }
                try { capture.Dispose(); }
                catch (Exception ex) { Logger.Warn(ex, "Failed to dispose visualizer capture"); }
            }
            var device = _renderDevice;
            _renderDevice = null;
            try { device?.Dispose(); }
            catch (Exception ex) { Logger.Warn(ex, "Failed to dispose visualizer endpoint"); }
        }
        ...
        private void OnDataAvailable(object? sender, WaveInEventArgs e)
        {
            ...
            int bytesPerSample = _bytesPerSample;
            ...
            for (int i = 0; i < samplesRecorded; i++)
            {
                ...
                if (_lastUpdateTime == DateTime.MinValue)
                    Logger.Info("Media widget visualizer received its first audio frame");
                _lastUpdateTime = now;
        ...
        private void OnRecordingStopped(object? sender, StoppedEventArgs e)
        {
            ...
            _isRunning = false;
            RequestRestart("audio recording stopped");
            if (e.Exception != null)
            {
